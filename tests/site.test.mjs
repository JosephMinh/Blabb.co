...
assert.match(phoneGenerator, /rounded_plate\([\s\S]*"PHONE_BODY", phone_width, phone_depth, phone_height, face_radius/);
...
assert.match(phoneGenerator, /face_inset = 0\.035/);
assert.match(phoneGenerator, /"DISPLAY_GLASS",[\s\S]*phone_width - face_inset \* 2,[\s\S]*phone_height - face_inset \* 2/);
...
assert.match(phoneScene, /PlaneGeometry\(3\.43, 7\.38\)/);
assert.match(phoneScene, /screenMesh\.position\.set\(0, 0, 0\.2148\)/);
assert.doesNotMatch(phoneScene, /screenMesh\.material\.polygonOffset/);
assert.match(screenTexture, /context\.roundRect\(0, 0, canvas\.width, canvas\.height, 46\)/);

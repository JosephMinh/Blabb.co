...
- One 1.7 MiB GLB with roughly 70,000 triangles and 91 named physical parts,
  generated by `tools/generate_phone_model.py`.
- One permanently assembled handset. Its rear shell, battery and coil,
  mainboard/local engine, midframe, rails, display, and glass remain named model
  groups for maintainability, but never separate in the presentation.
...
- A dimensional Three.js Blabb bubble with metal rim, state ring, stop/check/
  processing badges, touch ripples, local-flow particles, and a 3D snooze dock.
...
- Build the handset as an original Blender-generated GLB with named hardware
...
- Include named groups for shell, battery, mainboard/local engine, midframe,
  rails, display, and glass, but render them as one permanently assembled phone
  with a compressed, believable handset thickness.
- Raycast the visible phone for drag initiation. Use damped inertial yaw and a
  restrained pitch range; reserve horizontal touch motion for rotation and
  leave vertical touch motion to native scrolling.

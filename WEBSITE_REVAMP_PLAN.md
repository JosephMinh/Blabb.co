...
- One 574 KiB GLB with 20,464 triangles and 32 named exterior meshes,
  generated by `tools/generate_phone_model.py`.
- One permanently assembled handset built around a single continuous plum
  enclosure with a realistic 12%-of-width depth profile, glass and display
  matched to its exact front-face outline, rear finish, camera bar, and
  restrained edge buttons.
  The USB-C socket, microphone, and speaker holes are true shell recesses with
  dark interiors rather than protruding decoration. It contains no battery,
  board, midframe, display-bed, or other interior mesh.
...
- A shared depth profile compresses the complete bubble assembly to a slim 6%
  edge-on silhouette while preserving its full face-on shape and interactions.
...
- Use one continuous enclosure for the back, sides, corners, and front bezel.
  Embed the glass into a solid front face with no visible cavity behind it, cut
  bottom I/O into the enclosure, and limit protrusions to legitimate exterior
  details, chiefly the rear camera bar and tactile buttons. Do not model
  interior parts.
